...
/*** creates ***/
	/* createOne */
		...
		function createOne(REQUEST, RESPONSE, callback) {
			try {
				// session
					let session = CORE.getSchema("session")
		...
		function updateOne(REQUEST, RESPONSE, callback) {
			try {
				// query
					...
					if (REQUEST.updateSession && REQUEST.updateSession.composerId !== undefined) {
						query.document.composerId = REQUEST.updateSession.composerId
					}
					if (REQUEST.updateSession && REQUEST.updateSession.musicId !== undefined) {
						query.document.musicId = REQUEST.updateSession.musicId
					}
				...
				CORE.logError(error)
				callback({success: false, message: "uanble to " + arguments.callee.name})
			}
		}
