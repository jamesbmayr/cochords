				...
				// get
					if (REQUEST.method == "GET") {
						switch (true) {
							...
							// music
								case (/^\/music\/[a-zA-Z0-9]{8}$/).test(REQUEST.url):
									try {
										RESPONSE.writeHead(200, CORE.constructHeaders(REQUEST))
										CORE.renderHTML(REQUEST, "./html/music.html", function (html) {
											RESPONSE.end(html)
										})
									}
									catch (error) {_404(REQUEST, RESPONSE, error)}
							...
							// home
								// createMusic
									case "createMusic":
										try {
											RESPONSE.writeHead(200, CORE.constructHeaders(REQUEST))
											MUSIC.createOne(REQUEST, function (data) {
...
/*** socket ***/
		...
		function saveSocket(REQUEST) {
			try {
				// on connect - save connection & fetch music
					...
					sendSocketData({musicId: REQUEST.path[REQUEST.path.length - 1], success: true, message: "connected", recipients: [REQUEST.session.id]})
					MUSIC.readOne(REQUEST, sendSocketData)
				...
				// on close
					REQUEST.connection.on("close", function (reasonCode, description) {
						...
						// update music
							REQUEST.post = {action: "disconnectComposer"}
	...
	/* sendSocketData */
		function sendSocketData(data) {
			try {
				// hunt down errors
					if (!data.recipients || !data.musicId) {
						return
					}
				...
				// loop through recipients
					for (let r in recipients) {
						try {
							if (CONNECTIONS[recipients[r]] && CONNECTIONS[recipients[r]][musicId]) {
								CONNECTIONS[recipients[r]][musicId].sendUTF(data)
							}
						}
						catch (error) {CORE.logError(error)}
					}
